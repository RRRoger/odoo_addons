    ...
    for val in records:
        if val and info.get(val[0], False):
            if val[0] == 0:
                ...
                if obj._fields[r_name].type == 'many2one':
                    inst = getattr(obj, r_name).browse(value.get(r_name))
                    inner_msg = inst and inst.name_get()
                    inner_msg = inner_msg and inner_msg[0][1]
                else:
    ...
    def _add_write_patch_model(self):
        ...
        for record in self:
            ...
            if not obj._fields.get('message_ids', False):
                raise UserError(_('This model does not have a relation '
